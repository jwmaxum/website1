...
본 문서는 ONEDAYS BEAUTY 웹사이트 및 커머스 백오피스의 **전체 관리자 메뉴별 상세 기능, 연동 아키텍처 및 운용 지침**을 완벽하게 정리한 통합 설명서입니다.
...
- **브랜드명 관리**: 한국어 브랜드명(예: `원데이즈뷰티`) 및 영문 브랜드명(`ONEDAYS BEAUTY`) 설정 (전역 파비콘/헤더/Footer 자동 반영).
